---
title: "UNIVAF appointment availability data - Colorado"
...
* On a zip-code level, social vulnerability is _not_ strongly correlated with appointment availability.
...
x = Dav %>% filter(!is.na(provider)) %>% group_by(provider, work_time) %>% summarize(stat=n()) %>% ungroup()

x %>% mutate(provider=factor(provider, levels=x %>% filter(work_time==0) %>% arrange(-stat) %>% .$provider),
             work_time=factor(work_time, labels=c("Outside\nwork","During\nwork"))) %>% 
  ggplot(aes(provider, stat, color=work_time)) + geom_point() +
    scale_y_continuous("Total number of slots") + scale_x_discrete("Provider") +
...
```{r, fig.height=3, fig.width=6}
x = Dav %>% filter(!is.na(provider)) %>% group_by(provider, work_time) %>% summarize(stat=mean(range, na.rm = T)) %>% ungroup()

x %>% mutate(provider=factor(provider, levels=x %>% filter(work_time==0) %>% arrange(-stat) %>% .$provider),
             work_time=factor(work_time, labels=c("Outside\nwork","During\nwork"))) %>% 
  ggplot(aes(provider, stat, color=work_time)) + geom_point() +
    scale_y_continuous("Avg hours slots are available for") +
    scale_x_discrete("Provider") +
    my_theme() + theme(axis.text.x=element_text(angle=40, vjust=0.6), legend.position=c(0.9, 0.9), axis.title.x = element_blank())
```
...
* From this view is looks like appointments are last-minute bookable including outside working hours.
...
```{r, fig.height=3, fig.width=6}
x = Dav %>% filter(!is.na(provider)) %>% group_by(provider, work_time) %>% summarize(stat=mean(last_time_ahead, na.rm = T)) %>% ungroup()
...
Stats = stats %>%
  ...
  filter(!(var_y=='slots_per_person' & (val_y > 0 | val_y  < -5))) %>%
  filter(!(var_y=='last_available' & val_y > 80)) %>%
  filter(!(var_x=='p_black' & val_y < -7)) %>%
...
For most indices, we the trend in `r S` is similar to the federal one. Generally, `r S` has better stats though: more slots/person, longer availability, but also less last-minute availabilty.
...
* **Availability stats are somewhaat predictive of the number of vaccinations**. More (adjusted) slots/person correlates with more vaccinations and a lower max lead time as well.

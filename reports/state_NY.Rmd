---
title: "USDR Appointment Availability Data - New York"
...
dav  = read_availability_slot_data() %>% inner_join(dloc, by='id') %>%
       mutate(ifelse(last_time_ahead < 0, 0, last_time_ahead)) %>%
...
* Zip-codes with a higher social vulnerability index have less appointment availability. In New York this is worse than the national average for minority zip-codes.
...
```{r}
table(Dav %>% filter(provider != 'community_a_walgreens_pharmacy') %>% .$provider)
```
...
* most counties seem to have an approximately 20% coverage rate.
...
x %>%
  ...
  ggplot(aes(county, stat, color=work_time)) + geom_point() +
    ...
    my_theme() + theme(axis.text.x=element_text(angle=90), legend.position=c(0.9, 0.9), axis.title.x = element_blank())
```
...
x %>%
  ...
  ggplot(aes(county, stat, color=work_time)) + geom_point() +
    ...
    scale_x_discrete("County") +
    my_theme() + theme(axis.text.x=element_text(angle=90), legend.position=c(0.9, 0.2), axis.title.x = element_blank())
```
...
For most indices, we see a **negative correlation between vulnerability and availability** (household SVI excluded), and the trend for `r S` is a worse than the national average for the share Black and Minority SVI dimensions. 

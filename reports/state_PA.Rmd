---
title: "USDR Appointment Availability Data - Pennsylvania"
...
dagg = make_regression_data(dav, dzip, dvax, dcou)
...
Most counties seem to have an approximately 5-10% coverage rate.
  ...
  if(dow) {
    ...
    p = p + 
          scale_y_reverse("Date",
                          breaks=as.numeric(as.Date(c('2021-05-17', '2021-05-24', '2021-05-31', '2021-06-07', '2021-06-14', '2021-06-21'))),
                          labels=c('May 17', 'May 24', 'May 31', 'Jun 07', 'Jun 14', 'Jun 21'))
  }
...
```{r, fig.height=7.4, fig.width=6.2}
...
* Availability has been fairly consistent
...
* Statistics will be driven by Walgreens.
...
* Morning and weekend slots are available for the least amount of time. 
* Evening slots are available for longer. This is counter to the finding for most states.
...
```{r, fig.height=7.2, fig.width=6.2}
...
```{r}
dav %>% group_by(type) %>%
  summarize(n=n(), p_booked=mean(booked), range=mean(range), last_time_ahead=mean(last_time_ahead))
```

In `r S`, we see;

* weekend slots are booked at a much higher rate than weekday slots
* slots outside the working day are available for less time, and you have to book them further in advance
...
stargazer::stargazer(
  ...
  #lm(n_vax/population ~ hesitant + old_vax_rate + slots_per_person_workday + slots_per_person_weekend + slots_per_person_evening
  #                       + avg_range_workday + avg_range_weekend + avg_range_evening
  #                       + avg_ahead_workday + avg_ahead_weekend + avg_ahead_evening, data=Dagg),
...
* **Availability stats are predictive of the number of vaccinations**, and explain about 4% of the variation.

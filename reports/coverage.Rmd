...
vts_e = read_csv("../../data/clean/ids_external.csv", col_names = c("external_id",'iid')) %>% separate(external_id, into=c('key','val'), sep=':')
```
...
dzip %>%
  ...
  left_join(
    ...
    dloc %>% select(iid=id, uuid) %>%
      left_join(inner_join(vts_c, vts_e, by=c('key','val')) %>% select(vts_id, iid) %>% distinct() %>%
            group_by(iid) %>% summarize(vts_id=paste(vts_id, collapse=',')), by='iid') %>%
